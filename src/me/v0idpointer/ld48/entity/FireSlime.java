...
public class FireSlime extends Entity {
    ...
    public FireSlime(int x, int y, String name) {
        super(11, x, y, name);
        this.setHasAi(true);
        ...
        if(this.jumpTime > 14) {
            for(Entity entity : Game.instance.getDisplayComponent().world.getEntities()) {
                if(entity.getHitbox().intersects(this.getHitbox()) && entity.getId() == 1) {
                    ...
                    player.setBurningTimer((new Random().nextInt(3)) * 64);
    ...
    public void render(Graphics g) {
        g.drawImage(SpriteSheet.TERRAIN.getSprite( ( (this.getVelX() > 0 || this.getVelY() > 0) ? 5 : (( (this.animationTimer > 16) ? 4 : 6 )) ) , 3), this.getX(), this.getY() , 48, 48, null);
    }
